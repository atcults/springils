...
import org.sanelib.ils.api.converters.agency.AddAgencyConverter;
...
public class AddBindingTypeConverterTest {
    ...
    public void testAddBindingTypeSuccessExecute() throws Exception{
        ...
        assertEquals("Price is not mapped", dto.getPrice(), String.valueOf(addBindingType.getPrice()));
        assertEquals("Entry Id is not mapped", dto.getEntryId(), addBindingType.getEntryId());
        assertEquals("Entry date is not mapped", dto.getEntryDate(), DateHelper.toDateString(addBindingType.getEntryDate()));
